...
# --- UPDATED: Construct paths to models in the parent directory ---
# This ensures the script can find the model files regardless of where it's called from.
base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
predictor_path = os.path.join(base_dir, 'job_title_predictor_model.pkl')
...
try:
    job_title_predictor = joblib.load(predictor_path)
    tfidf_vectorizer = joblib.load(vectorizer_path)
    print("Models loaded successfully.")
except FileNotFoundError as e:
    print(f"Error: ML model file not found at {e.filename}. Please ensure all model files are in the main 'backend' directory.")
    exit()
...
def extract_resume_details(text):
    ...
    # Vectorize the cleaned text using the new TF-IDF vectorizer
    text_vector = tfidf_vectorizer.transform([text_lower])
    # Make a prediction using the new job title predictor model
    predicted_category = job_title_predictor.predict(text_vector)[0]
    ...
    # --- UPDATED: IMPROVED EXPERIENCE EXTRACTION LOGIC ---
    total_experience_months = 0
    date_pattern = re.compile(r'(\w{3}\s+\d{4})\s*-\s*(\w{3}\s+\d{4}|present)', re.I)
    
    matches = date_pattern.finditer(text_lower)
    ...
    for match in matches:
        ...
        try:
            start_date = date.fromisoformat(start_date_str.replace(' ', '-'))
            
            if end_date_str.lower() == 'present':
                end_date = date.today()
            else:
                end_date = date.fromisoformat(end_date_str.replace(' ', '-'))
            
            # Calculate months between start and end dates
            delta = relativedelta(end_date, start_date)
            total_experience_months += delta.years * 12 + delta.months
        except ValueError:
            pass # Skip malformed date ranges
            
    # Add simple year-based experience if no date ranges are found
    if total_experience_months == 0:
        matches_years = re.findall(r'(\d+(?:\.\d+)?)\+?\s*(?:year(?:s)?|yr(?:s)?)\b', text_lower)
        if matches_years:
            total_experience = sum([float(m) for m in matches_years])
        ...
        if total_experience_years < 1.0:
            experience = f"Less than 1 Year ({total_experience_months} months)"
        else:
            experience = f"{round(total_experience_years, 1)} Years"

    # --- END OF UPDATED SECTION ---

    ats_score = 40
    if "experience" in text_lower or experience != "Not Found (Ensure you list your work experience with clear start and end dates or total years.)": ats_score += 15
    ...
    if experience == "Not Found (Ensure you list your work experience with clear start and end dates or total years.)":
